...
func TestOpenAPI(t *testing.T) {
	...
	filepath.Walk(root, func(path string, info os.FileInfo, err error) error {
		...
		t.Run(path, func(t *testing.T) {
			spec, err := openapi3.ParseYAML(rdr)
...
func TestQuery(t *testing.T) {
	...
	spec, err := openapi3.ParseYAML(bytes.NewReader(src))
	...
	if !assert.Implements(t, (*openapi3.Schema)(nil), v, "The result should be a schema") {
		return
	}
}
...
`
	...
	if !assert.NoError(t, err, `openapi3.ParseYAML should succeed`) {
		return
	}
